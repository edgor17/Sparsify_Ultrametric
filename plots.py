...
for item in np.unique(depths):
    c=next(color)
    unifracplot=ax[0].scatter(unifraccoord[np.where(depths==item),0] ,unifraccoord[np.where(depths==item),1],c=c,label=str(item/1000))
    DPCoAplot=ax[1].scatter(-DPCoAcoord[np.where(depths==item),0] ,DPCoAcoord[np.where(depths==item),1],c=c,label=str(item/1000))
    haarlikeplot=ax[2].scatter(-haarlikecoord[np.where(depths==item),0] ,haarlikecoord[np.where(depths==item),1],c=c,label=str(item/1000))
